...
- [x] Fazer a primeira coleta real no Samsung: a execução 6 publicou 1.169 produtos, 33 páginas e 1.169 medições com qualidade `completa`; a tentativa concorrente 7 falhou com código `acesso` sem substituir esse snapshot.
...
- [x] Instalar Termux, Termux:Boot, Termux:API, Python, Appium/UiAutomator2 e Chrome no Samsung e aprovar diagnóstico e coleta completa: `total=1169`, 36 itens na primeira página, 33 páginas e 1.169 itens publicados. O driver puro `psycopg` foi instalado no ARM32 contra o `libpq` local.
...
- [ ] Fechar o caminho rápido Android com 1.169 itens únicos e zero duplicados: a execução 18 teve 1.163 únicos/6 duplicados e a 19 foi recusada como parcial pelo gate de integridade; depois fazer três execuções consecutivas, validar o receiver do Termux:Boot após reinicialização e conferir API/Flutter.
