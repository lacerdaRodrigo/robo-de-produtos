...
| CT-375 | Adaptador Android isolado | Appium local abre ou relança o Chrome nativo com UiAutomator2, abre a URL permitida e aguarda uma aba DevTools quando o processo antigo não publica DevTools; o CDP local via ADB lê `products.items`, usa `pageSize=200`, exige a faixa completa renderizada inclusive na última página, preserva o mesmo parser e encerra a ponte/driver; URL remota é rejeitada | `teste_pichau.py` |
...
| CT-385 | Resultado do worker | Saída zero do `pichau-android-run.sh` encerra a fila como `sucesso`; falhas usam categorias seguras de ADB, Appium, configuração, navegador, acesso, dados, banco, parcial ou inesperada | `teste_fila_android.py` e `teste_pichau.py` |
...
| CT-393 | Inicialização transitória do DevTools | Resposta JSON parcial ou ausência momentânea de uma aba DevTools durante o boot do Chrome aguarda a estabilização dentro de limite finito antes de recriar a sessão Appium; a sessão UiAutomator2 nativa não recebe o timeout W3C `pageLoad` incompatível | `teste_pichau.py` |
...
| CT-396 | Worker foreground recuperável | O boot transfere o processo ao daemon, a pasta de boot contém somente lançadores ativos, o worker mantém wake/Wi-Fi lock sem o runner liberá-lo e o watchdog permissivo relança somente quando o flock está livre | análise estática dos scripts Termux e aceite operacional no Samsung |
| CT-397 | Diagnóstico local seguro | O status verifica checkout, configuração, worker, watchdog, fila e ADB sem imprimir URL do banco, host, porta ou serial; Appium ocioso é estado válido | `teste_fila_android.py`, análise estática dos scripts e aceite operacional no Samsung |
