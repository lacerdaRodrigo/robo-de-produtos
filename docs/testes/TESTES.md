...
| CT-390 | Descoberta ADB Wi-Fi | O runner reutiliza exatamente um endpoint `device` já conectado ou descobre exatamente um serviço Wireless Debugging pareado por mDNS, filtra o host privado e propaga o endpoint somente em memória; validado na execução real `34302348225` sem cabo USB | `pichau-android-run.sh` e validação operacional no Samsung |
...
| CT-396 | Worker foreground recuperável | O boot transfere o processo ao daemon, o worker mantém wake/Wi-Fi lock sem o runner liberá-lo e o watchdog permissivo relança somente quando o flock está livre | análise estática dos scripts Termux e aceite operacional no Samsung |
