...
| CT-194 | Link individual do Inter | O destino usa a rota oficial do `slug`, sem trocar o host aprovado | Comparar URL do card da C&A e testar codificação do slug |
...
| CT-293 | Histórico de 30 dias | Folha de histórico segue o padrão V11, mostra datas, preços, cashback e preço após cashback reais, mínimo, máximo, paginação, falha e retry sem converter valores em `double` nem usar exemplos fixos do protótipo | Mock autenticado da API e widget |
...
| CT-375 | Adaptador Android isolado | Cada coleta força o fechamento do Chrome, abre a URL permitida diretamente por ADB/CDP e usa Appium/UiAutomator2 somente como fallback; ao sair, encerra driver, confirma o Chrome parado e remove a ponte; URL remota é rejeitada | `teste_pichau.py` |
...
| CT-381 | Telemetria operacional segura | Runner, páginas, coleta e publicação registram duração e contagens, sem mensagem de exceção, HTML, título, URL, cookies, headers ou credenciais | `teste_pichau.py` e revisão do runner Termux |
| CT-383 | Fila Android idempotente | A mesma chave de workflow, formada pelo run ID e SHA disparador, retorna o trabalho existente; origens e chaves fora do contrato são rejeitadas sem expor a URL do banco | `teste_fila_android.py` |
...
| CT-385 | Resultado do worker | Saída zero do `pichau-android-run.sh` encerra a fila como `sucesso`; em falha, o código granular validado do diagnóstico substitui a categoria genérica quando existir | `teste_fila_android.py` e `teste_pichau.py` |
| CT-387 | Pré-voo Wi-Fi do executor Android | O runner exige o modo Wi-Fi, reutiliza ou descobre um endpoint pareado, valida `device` e falha sem expor o endpoint privado | `pichau-android-run.sh` e validação operacional no Samsung |
| CT-390 | Descoberta ADB Wi-Fi | O runner reutiliza exatamente um endpoint `device` já conectado, usa a porta fixa privada quando a build Termux não oferece mDNS ou descobre exatamente um serviço pareado; sempre filtra o host privado e propaga o endpoint somente em memória; validado na execução `34424475472` | `pichau-android-run.sh`, `pichau-android-status.sh` e validação operacional no Samsung |
...
| CT-401 | “Fechar tudo” sem coordenada | O adaptador e o trap enumeram somente tarefas recentes `type=standard`, limitam/validam IDs, removem cada uma com `am stack remove`, acionam `KEYCODE_HOME` e confirmam lista vazia; Home/Recents e conteúdo do `dumpsys` não entram nos logs | `teste_pichau.py`, `teste_fila_android.py` e validação real no Samsung |
| CT-402 | Checkout Android alinhado à `main` | Antes da coleta, o worker recusa alteração local versionada, busca `origin/main`, aceita somente fast-forward, confirma que o HEAD contém o SHA do workflow e reinstala projeto/extra Android quando o HEAD muda; falha segura como `pichau-checkout` sem iniciar o runner e mantém compatibilidade com chaves antigas | `teste_fila_android.py`, `pichau.yml` e revisão do fluxo Termux |
