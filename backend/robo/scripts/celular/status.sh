...
ok() {
    echo "robo-celular-status: $1=ok"
}

falha() {
    echo "robo-celular-status: $1=falha" >&2
    status=2
}

if commit="$(git -C "$REPO_ROOT" rev-parse --short HEAD 2>/dev/null)"; then
    echo "robo-celular-status: checkout=$commit"
else
    falha "checkout"
fi
...
if [[ -e "$LOCK_FILE" ]] && ! flock -n "$LOCK_FILE" -c true 2>/dev/null; then
    ok "worker"
    echo "robo-celular-status: wake-lock-owner=worker"
else
    falha "worker"
...
if [[ -n "${DATABASE_URL:-}" && -x "$VENV_DIR/bin/python" ]] \
    && DATABASE_URL="$DATABASE_URL" PYTHONPATH="$ROBO_ROOT/src${PYTHONPATH:+:$PYTHONPATH}" \
        "$VENV_DIR/bin/python" -m robo_celular.fila health >/dev/null 2>&1; then
    ok "fila-coletas"
else
    falha "fila-coletas"
fi
...
if "$ROBO_ROOT/scripts/pichau/appium.sh" status >/dev/null 2>&1; then
    echo "robo-celular-status: appium=ativo"
else
    echo "robo-celular-status: appium=ocioso"
fi
