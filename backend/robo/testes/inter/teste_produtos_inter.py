...
from robo_inter.adaptadores_produtos_inter import (
    EXPIRACAO_EXECUCAO_PRODUTOS,
    FonteProdutosInterHttp,
    RepositorioProdutosInterPostgres,
)
from robo_inter.extrator_produtos_inter import (
    RespostaProdutosInterInvalida,
    extrair_lojas_diretas,
    extrair_pagina_produtos,
    normalizar_busca_produtos,
)
from robo_inter.modelos_produtos_inter import (
    LojaDiretaInter,
    ProdutoDiretoInter,
    ResumoColetaProdutosInter,
)
from robo_inter.portas_produtos_inter import FalhaProdutosInter, PaginacaoProdutosInterInvalida
from robo_inter.principal_produtos_inter import coletar_produtos_de_loja, preparar_rodada
...
def teste_conclusao_da_rodada_gera_alertas_depois_do_estado_final(monkeypatch):
    ...
    assert [
        parametros for comando, parametros in cursor.comandos if comando == repositorio.GERA_ALERTAS
    ] == [
        (71,),
        (72,),
    ]
